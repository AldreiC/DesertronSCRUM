...
def control_robot():
    ...
    if action == 'backward':
        move_forward(.25, speed)  # Move forward for 2 seconds
        return "Moving backward", 200

    elif action == 'forward':
        move_backward(.25, speed)  # Move backward for 2 seconds
        return "Moving forward", 200

    elif action == 'right':
        turn_left(.25, speed)  # Default duration is 1 second
        return "Turning right", 200

    elif action == 'left':
        turn_right(.25, speed)  # Default duration is 1 second
        ...
        FTturn_left(.05,speed)
    elif action == "FTRight":
        FTturn_right(.05,speed)
        pass
